...
filebox = ['E:\课题组资料\5-汇报类\组会\20231209-夏月星实验汇报\20231205\bapbi-3-124uW'];
file = search_folder(filebox,'dat');
n_max = length(file);

%% 给出最强点的坐标和需要删除的边框，根据得到的结果略微调整

xl = 3;   xr = 55; yd = 140; yu = 320; % xleft,xright,ydown,yup;
xc = (xl+xr)/2; yc = (yd+yu)/2; % xcenter,ycenter;
px = 11; py = 10; msdt =1.6; msdp =20;
...
for ii = 1:n_max
    ...
    decay_raw = str2double(temp_file((st_position(end)+1):dot_position(end)-1)); % 读入的时间为四位数
...
%     ini_0 = [A xc yc z0 w Ac k u];                                                    % 参数顺序
%     ini_0 = [A xc yc z0 w]; 
    ini_0 = [A xc yc z0 w];
    guass_fun =@(p,q) ...
        p(4)+(p(1)./(p(5).*sqrt(pi./2))).*exp(-2.*((q(:,1)-p(2)).^2+(q(:,2)-p(3)).^2)./p(5).^2);
%        +p(6).*cos(p(7).*sqrt((q(:,1)-p(2)).^2+(q(:,2)-p(3)).^2)+p(8));     % 拟合函数为y = y0 + (A/w*sqrt(pi/2))*exp(-2((x-xc)/w)^2)
