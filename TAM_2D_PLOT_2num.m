...
time_zero = 1985;   % 时间零点t0，可以自行设置也可以在line(32,34)读入时设置

spacing = 0.0352360817477097;                                               % 1 pixel = 0.0352360817477097 μm

filebox = 'E:\CODE\Note_file\20230831\s1-cpb-sio2-p=11uw';
file = search_folder(filebox,'dat');
n_max = length(file);

%% 给出最强点的坐标和需要删除的边框，根据得到的结果略微调整

xl = 1;   xr = 59; yd = 52; yu = 174; % xleft,xright,ydown,yup;
xc = (xl+xr)/2; yc = (yd+yu)/2; % xcenter,ycenter;
px = 6; py = 9; msdt =31; msdp = 600;

msd = zeros(n_max,1);                                                       % 均方位移σ^2
msd2 = zeros(n_max,1);                                                       % 均方位移σ^2
...
for ii = 1:n_max
    ...
    A = 0.1;  xc = 0; yc = 0; z0 = 0; w = 1;                                    % 初始参数，需要一定的调整
    ini_0 = [A xc-0.003 yc z0 w -0.1*A xc+0.0001 yc w];                                                    % 参数顺序
    
    guass_fun =@(p,q) ...
        p(4)+(p(1)./(p(5).*sqrt(pi./2))).*exp(-2.*((q(:,1)-p(2)).^2+(q(:,2)-p(3)).^2.)./p(5).^2)...
        +(p(6)./(p(9).*sqrt(pi./2))).*exp(-2.*((q(:,1)-p(7)).^2+(q(:,2)-p(8)).^2.)./p(9).^2);     % 拟合函数为y = y0 + (A/w*sqrt(pi/2))*exp(-2((x-xc)/w)^2)
%     options = optimoptions('lsqcurvefit','Algorithm','levenberg-marquardt');% 非线性最小二乘法使用levenberg-marquardt算法
    para = lsqcurvefit(guass_fun,ini_0,[xfit,yfit],zfit,[0,-1,-1,-0.01,0.1,-10,-1,-1,0.1],[20,1,1,0.01,4,0,1,1,10]);% lsqcurvefit(函数，参数初始值，横坐标，纵坐标）返回拟合参数值               
    
    %% 数据处理，画图并获取msd
    A = para(1); xc = para(2); yc = para(3); z0 = para(4); w = para(5); w2 = para(9);                    % 获取对应值
...
plot(delay_time,msd,'ro');    hold on
plot(delay_time(sr:so,1),polyval(k_para,delay_time(sr:so,1)),'r-');

k2_para = polyfit(delay_time(sr:so,1),msd2(sr:so,1),1);
plot(delay_time,msd2,'bo');    hold on
plot(delay_time(sr:so,1),polyval(k2_para,delay_time(sr:so,1)),'b-');

mobilty = k_para(1)*193400                                                  % 得到最后的迁移率
mobilty2 = k2_para(1)*193400                                                  % 得到最后的迁移率
